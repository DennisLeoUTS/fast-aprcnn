...
LOG="experiments/logs/baseline_oridetect_oridata_retrain_prcnn_vgg16.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
time ./tools/train_prcnn.py --gpu $1 \
  ...
  --imdb_test cub_test \
  --imdb_weak cub_weak \
  --cfg experiments/cfgs/baseline_oridetect_oridata_retrain_prcnn.yml
