...
class cub_weak(datasets.imdb):
    ...
    def gt_roidb(self):
        ...
        if cfg.PRCNN.NOISE_REMOVE:
            sel_file = os.path.join(cfg.PRCNN.WEAK_DIR, 'selected_samples.pkl')
        ...
        for i in range(self.num_parts):
            ...
            with open(cache_file, 'rb') as fid:
                ...
                if i==0: valid_ind = [False for _ in xrange(num_images)]
                
                out_boxes = []
                for j in xrange(num_images):
                    if cfg.PRCNN.NOISE_REMOVE:
                        if selected_inds_all[i].count(j)>0:
        ...
        #img_index_valid = [image_index[j] for j in range(len(image_index)) if valid_ind[j]]
        #print len(img_index_valid)
        #print len(self.gt_list)
        
        self._image_index = [self._image_index[i] for i in xrange(len(valid_ind)) if valid_ind[i]]
        self._image_labels = [self._image_labels[i] for i in xrange(len(valid_ind)) if valid_ind[i]]
        for tmp in xrange(len(self.gt_list)):
            self.gt_list[tmp] = [self.gt_list[tmp][i] for i in xrange(len(valid_ind)) if valid_ind[i]]
        
        gt_roidb = [self._load_annotation_helper(i)
                    for i in range(len(self._image_index))]
        
        print 'Num images after noise removal:', self.num_images
        self.valid_ind = valid_ind
        
        if cfg.PRCNN.NOISE_REMOVE:
            cache_file = os.path.join(self.cache_path, self.name + '_gt_roidb.pkl')
            valid_file = os.path.join(self.cache_path, self.name + '_valid_ind.pkl')
            with open(valid_file, 'wb') as fid:
                cPickle.dump(valid_ind, fid, cPickle.HIGHEST_PROTOCOL)
        else:
            cache_file = os.path.join(self.cache_path, self.name + '_gt_roidb_no_denoise.pkl')
    ...
    def selective_search_roidb(self):
        ...
        if cfg.PRCNN.PHRASE == 'train':       # only for testing
            ...
            if cfg.PRCNN.NOISE_REMOVE:
                ...
                cache_file = os.path.join(self.cache_path,
                                  self.name + '_selective_search_roidb_retrain_nodenoise.pkl')
